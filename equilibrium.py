...
def make_u_y(loads):
    ...
    for i in range(loads.hinge2_idx - 1, -1, -1):
        ...
        u[i, 0] += -loads.pointload_defl(shearforces[i, 0], loads.dx_list[i], loads.a.Izz()) + \
                   loads.moment_defl(moments[i, 0], loads.dx_list[i], loads.a.Izz())
        # Angle due to aero load
        thetas[i, 0] = loads.forward_angle_distr(i)
        # Angle due to internal shear and moment due to aero load
        thetas[i, 0] += loads.moment_angle(moments[i, 0], loads.dx_list[i], loads.a.Izz()) - \
                        loads.pointload_angle(shearforces[i, 0], loads.dx_list[i], loads.a.Izz())
        ...
        if loads.bending:
            ...
            u[i, 2] += -loads.pointload_defl(shearforces[i, 1], loads.dx_list[i], loads.a.Izz()) + \
                       loads.moment_defl(moments[i, 1], loads.dx_list[i], loads.a.Izz())
            # Angle due to force 1
            thetas[i, 1] = loads.forward_angle_point(i)
            # Angle due to internal shear and moment due to force 1
            thetas[i, 1] += -loads.pointload_angle(shearforces[i, 1], loads.dx_list[i], loads.a.Izz()) + \
                           loads.moment_angle(moments[i, 1], loads.dx_list[i], loads.a.Izz())
    ...
    for i in range(loads.hinge2_idx + 1, loads.gridsize, 1):
        ...
        # Contribution of internal shear and moment due to aero load
        u[i, 0] += -loads.pointload_defl(shearforces[i-1, 0], loads.dx_list[i-1], loads.a.Izz()) + \
                   loads.moment_defl(moments[i-1, 0], loads.dx_list[i-1], loads.a.Izz())
        # Angle due to aero load
        thetas[i, 0] = loads.backward_angle_distr(i)
        # Angle due to internal shear and moment due to aero load
        thetas[i, 0] += loads.moment_angle(moments[i-1, 0], loads.dx_list[i-1], loads.a.Izz()) - \
                        loads.pointload_angle(shearforces[i-1, 0], loads.dx_list[i-1], loads.a.Izz())

        # Previous deflection due to angle at D and new contribution
        u[i, 1] += u[i-1, 1] + loads.dx_list[i - 1]

        u[i, 3] += u[i-1, 3] - loads.pointload_defl(shearforces[i-1, 2],
                                                   loads.dx_list[i-1],
                                                   loads.a.Izz()) + loads.moment_defl(moments[i-1, 2],
                                                   loads.dx_list[i-1], loads.a.Izz())
        thetas[i, 2] = -loads.pointload_angle(shearforces[i-1, 2], loads.dx_list[i-1], loads.a.Izz()) + \
                       loads.moment_angle(moments[i-1, 2], loads.dx_list[i-1], loads.a.Izz())

        if loads.bending:
            u[i, 4] += u[i - 1, 4] + loads.backward_defl_point(i)
            u[i, 4] += -loads.pointload_defl(shearforces[i-1, 3], loads.dx_list[i-1], loads.a.Izz()) + \
                       loads.moment_defl(moments[i-1, 3], loads.dx_list[i-1], loads.a.Izz())
...
def make_u_z(loads):
    ...
    for i in range(loads.hinge2_idx - 1, -1, -1):
        ...
        thetas[i, 0] = loads.pointload_angle(shearforces[i, 0], loads.dx_list[i], loads.a.Iyy()) - \
                      loads.moment_angle(moments[i, 0], loads.dx_list[i], loads.a.Iyy())
        ...
        if loads.bending:
            ...
            # Adding deflection due to internal shear and moment due to force 1
            u[i, 2] += loads.pointload_defl(shearforces[i, 1], loads.dx_list[i], loads.a.Iyy()) - \
                       loads.moment_defl(moments[i, 1], loads.dx_list[i], loads.a.Iyy())
            # Angle due to force 1
...
def calc_deflection_z(loads, u=np.nan):
    ...
    matrix[4, 1:] = np.array([loads.hinge1_val-loads.hinge2_val, loads.act1_val-loads.hinge2_val, 0,
                              -loads.hinge3_val-loads.hinge2_val])
